...
exports.handler = async function (event) {
  ...
  try {
    ...
    // ✅ Fetch barber hours if userId is provided
    let barberHoursMap = {};
    let barberWeekends = [];
    ...
    if (userId) {
      const { data: timeOffData, error: toError } = await supabase
        ...
        .eq("ghl_id", userId);
      if (toError) {
        console.error("Failed to fetch time off:", toError.message);
      } else {
    ...
    for (const day of daysToCheck) {
      ...
      if (userId) {
        ...
        // ✅ Skip if date falls under time off
        if (isDateInTimeOff(day)) {
          console.log(`Skipping ${dateKey} - barber time off`);
          continue;
        }
