      ...
      if (timeOffRows.length) {
        ...
        available = available.filter((s) => {
          ...
          return !timeOffRows.some((t) => {
            ...
            // Parse the time-off period
            const startTime = new Date(t["Event/Start"]).getTime()
            const endTime = new Date(t["Event/End"]).getTime()
        ...
        available = available.filter((s) => {
          ...
          return !timeBlockRows.some((tb) => {
            ...
            if (recurring) {
              ...
              const blockDateKey = blockDate.toLocaleDateString("sv-SE", {
                timeZone: "America/Denver",
              })
